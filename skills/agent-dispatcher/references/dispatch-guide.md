# `agd dispatch` — flag reference and examples
...
agd dispatch -p "Summarise this PR diff."

# From a file.
agd dispatch -f /tmp/prompt.md

# Force a specific agent.
agd dispatch --agent claude-default -p "..."

# Force a tier (e.g. fallback tier).
agd dispatch --tier secondary -p "..."

# Inspect the resolved command without running.
agd dispatch --dry-run -p "..."

# Use a one-off config.
agd dispatch --config /tmp/test.toml -p "..."
```
...
|---|---|---|
| `agent not found` / unexpected agent selected | Config tier order or agent enabled flag. | `agd config show`; see config-guide.md. |
| Agent stalls / no output | Missing permission-bypass flag for that CLI. | config-guide.md "Permission-bypass flags" table. |
| Detect lists an agent but dispatch reports it as unavailable | `verified == false` in the upstream template, or `callable == false`. | `agd detect`; see detect-guide.md. |
