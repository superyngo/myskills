# `agd config` — schema, semantics, and skill behaviour
...
- **User-level:** `~/.config/agd.toml`
- **Project-level:** `<git-root>/.config/agd.toml` (if cwd is in a git repo)

The CLI's resolution order is: `--config PATH` if supplied → project-level if in a repo → user-level. The `agd init` JSON `save_location` field (`"user"` or `"project"`) selects which file `init` writes to. **`--config` does not redirect `init` output** — only `save_location` does.
...
| Command | Behaviour |
|---|---|
| `agd config show` | Forwarded. Prints the resolved TOML to stdout. |
| `agd config list` | Forwarded. Lists agents and their callable / enabled status. |
| `agd config path` | Forwarded. Prints the resolved config path. |
| `agd config edit` | **Intercepted by the skill.** Requires a TTY; fails with `Device not configured (os error 6)` inside the Bash tool. |
| `agd config` (no action) | **Intercepted by the skill.** Same TTY failure mode. |

## Editing the config

When you ask for `config edit` (or bare `config`), the skill instead:

1. Runs `agd config path` to capture the resolved path.
