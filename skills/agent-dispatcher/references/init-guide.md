# `agd init` — JSON stdin schema and skill orchestration

Loaded by the skill when the user invokes `init`. The CLI's `init` subcommand reads a JSON payload from stdin and writes a TOML config to either `~/.config/agd.toml` (`save_location: "user"`) or `<git-root>/.config/agd.toml` (`save_location: "project"`).

**The CLI overwrites existing config without warning.** The skill's orchestration adds an overwrite check (Backup / Overwrite / Cancel) before piping. Users who run `agd init` directly, outside the skill, do NOT get this guard.
...
| `agents[].cli` | string | Should match an `agd detect` output key. **The CLI does NOT validate** — any string is accepted. |
...
6. **Overwrite check** (runs after step 5 because it needs the chosen `save_location`):
   - Compute target path: `~/.config/agd.toml` for user, `<git-root>/.config/agd.toml` for project (`git rev-parse --show-toplevel`, fall back to cwd).
...
7. Pipe the JSON via stdin: `printf '%s' "<json>" | agd init`.
...
8. Run `agd config show` and surface the resulting file.
   - If you opted into bypass flags, the skill also runs `agd dispatch --dry-run` to show the resulting command shape.

## Tuning after init

Init seeds a minimal config (no extra `args` beyond bypass flags, no `env`). To add more args or env injection:

- Run `agd config path` to get the config location.
